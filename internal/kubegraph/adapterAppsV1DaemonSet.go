...
func (adapter adapterAppsV1DaemonSet) GetType() reflect.Type {
	return reflect.TypeOf(&appsv1.DaemonSet{})
}

func (adapter adapterAppsV1DaemonSet) Create(kgraph KubeGraph, obj k8sruntime.Object) (*cgraph.Node, error) {
	...
	return kgraph.addStyledNode(adapter.GetType(), obj, name, resource.Name, "icons/ds.svg")
}

func (adapter adapterAppsV1DaemonSet) Connect(kgraph KubeGraph, source *cgraph.Node, targetName string) (*cgraph.Edge, error) {
	return kgraph.linkNode(source, adapter.GetType(), targetName)
}

func (adapter adapterAppsV1DaemonSet) Configure(kgraph KubeGraph) error {
