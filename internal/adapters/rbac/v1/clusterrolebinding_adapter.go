...
// ClusterRoleBindingObject alias for rbacV1.ClusterRoleBinding
type ClusterRoleBindingObject = rbacV1.ClusterRoleBinding

// ClusterRoleBindingAdapterObjectType reflected type of *rbacV1.ClusterRoleBinding
var ClusterRoleBindingAdapterObjectType = reflect.TypeOf((*rbacV1.ClusterRoleBinding)(nil))
...
func GetClusterRoleBindingAdapter() (*ClusterRoleBindingAdapter, error) {
	adapter, err := registry.GetAdapter[*rbacV1.ClusterRoleBinding]()
	if err != nil {
		return nil, err
	}
...
// ClusterRoleBindingAdapter implements an adapter for rbacV1.ClusterRoleBinding
type ClusterRoleBindingAdapter struct {
	registry.Adapter
}

// CastObject casts a runtime.Object to *rbacV1.ClusterRoleBinding
func (this *ClusterRoleBindingAdapter) CastObject(obj registry.RuntimeObject) (*rbacV1.ClusterRoleBinding, error) {
...
// GetGraphObjects returns a map of all *rbacV1.ClusterRoleBinding
// nodes stored on a graph
func (this *ClusterRoleBindingAdapter) GetGraphObjects(statefulGraph registry.StatefulGraph) (map[string]*rbacV1.ClusterRoleBinding, error) {
	objects, err := statefulGraph.GetObjects(ClusterRoleBindingAdapterObjectType)
	if err != nil {
		return nil, err
	}

	castedObjects := make(map[string]*rbacV1.ClusterRoleBinding, len(objects))
	for key, object := range objects {
		casted, ok := object.(*rbacV1.ClusterRoleBinding)
		if !ok {
...
// GetGraphNode returns the node value representing a *rbacV1.ClusterRoleBinding
